...
suite("DemoGasOptimized", async function () {
  const { viem } = await network.connect();
  let contract = await viem.deployContract("DemoGasOptimized", [
    "DemoGasOptimized",
  ]);
  ...
  beforeEach(async function () {
    contract = await viem.deployContract("DemoGasOptimized", [
      "DemoGasOptimized",
    ]);
  });
  ...
  test("Should show name", async function () {
    ...
    assert.equal(name, "DemoGasOptimized");
  });
